...
Все запросы идут **напрямую из браузера** по HTTPS. API-ключи задаются администратором один раз в переменных окружения Vercel и работают на всех устройствах — **поле ручного ввода ключей отсутствует**, приложение готово к работе из коробки. Backend не используется.
...
При заданных env-ключах приложение готово к работе сразу — пользователю не нужно ничего вводить. Ключи встраиваются в сборку Vite (`VITE_`-префикс) и доступны на всех устройствах. **Поля ручного ввода ключей в интерфейсе нет** — это сделано намеренно для запуска «из коробки».
